...
type
  ...
  protected
    ...
    function ReadDisplayOption: boolean;
...
uses About, ChangePassword, HtmlHlp, Login,
  ...
  TrsItemShipment, MySQLConnector,
  trsPurchaseOrderList, trsPurchaseOrder, MstRelationList, trsReceipt,
  trsRetur, MstBarcode, trsPembayaran, trsPembayaranList, ReportStok,
  ReportItemQty, ReportDebt, ReportProfit, trsAkun,
  MstProduk, ReportPenjualan, ReportItemMovement,
  MstPaketList, trsSales2, trsSalesList, MstRak;
...
procedure InitializeApplication;
begin
  if MODUL_PATH = GLOBAL_REG_PATH_ELSINTA then begin
     Application.Name     := 'sMart';
     Application.HelpFile := 'sMart.chm';
     Application.Title    := 'sMart' +IfThen(DEMO_VERSION, ' [demo version]');
  end

  else if MODUL_PATH = GLOBAL_REG_PATH_RAJA then begin
     Application.Name     := 'sMart';
     Application.HelpFile := 'sMart.chm';
     Application.Title    := 'sMart' +IfThen(DEMO_VERSION, ' [demo version]');
  end

  else if MODUL_PATH = GLOBAL_REG_PATH_DELTA then begin
     Application.Name     := 'doapart';
     Application.HelpFile := 'doapart.chm';
     Application.Title    := 'doapart' +IfThen(DEMO_VERSION, ' [demo version]');
  end
  else begin
     Application.Name     := 'demo';
     Application.HelpFile := 'demo.chm';
     Application.Title    := 'smart' +IfThen(DEMO_VERSION, ' [demo version]');
  end
end;
...
//kasih warna default
  ...
  clFixedColum     := $00605155;
  clFixedFont      := $00FCFCFC;
  clPrimaryBand    := $00F3F3F3;
  clSecondaryBand  := $00FEFFFF;
  clSelection      := $00DADCFE;//$00E4E4E4;
  clGrid           := $00FAF8F9;
...
begin
  ...
  if (GlobalSystemUser.UserID='') then begin
    ...
    imgBanner.Picture.Bitmap.Handle:= LoadBitmap(ImageDll, 'baner_panjang');
    lblAboutClick(nil);
  end;
end;
...
begin
  if ReadDisplayOption = True then begin
    frmMainMenu.AlphaBlend:= True;
    frmMainMenu.AlphaBlendValue:= 0;
    if MenuAlignment = 'suiLeft' then SideMenu.Align:= suiLeft
    else SideMenu.Align:= suiRight;
  end;
...
procedure TfrmMainMenu.Timer2Timer(Sender: TObject);
begin
  if ReadDisplayOption = False then begin
    Timer2.Enabled:= False;
    frmMainMenu.AlphaBlend:= False;
    exit;
  end;

  if Timer2.Tag < 255 then begin
    Timer2.Tag := Timer2.Tag + 15;
    if Timer2.Tag > 255 then
      frmMainMenu.AlphaBlendValue:= 255
    else frmMainMenu.AlphaBlendValue:= Timer2.Tag;
  end;
...
function TfrmMainMenu.ReadDisplayOption: boolean;
var reg: TRegistry;
begin
  reg:= TRegistry.Create;
  try
    reg.OpenKey(MODUL_PATH+'\Option\Display\Screen', false);
    Result:= reg.ReadInteger('flashScreen') = -1;
    MenuAlignment:= reg.ReadString('menuAlignment');
  except
    reg.CloseKey;
    try
      reg.OpenKey(MODUL_PATH+'\Option\Display\Screen', True);
      reg.WriteInteger('flashScreen', 0);
      reg.WriteString('menuAlignment', 'suiLeft');

      Result:= False;
      MenuAlignment:= 'suiLeft';
    except
       raise;
    end;
  end;
  ...
  if (SystemMenu.FormType in [ftMaster,ftTransaction, ftReport,ftSetting])
    ...
    if not (Can_Access(Form, AMenuID)) then  Exit;
      ...
      case AMenuID of
        ...
        321: Form.Execute(ITEM_TYPE_INPUT); //frmTrsItemShipmentList
        322: Form.Execute(ITEM_TYPE_MUTASI); //frmTrsItemShipmentList
        323: Form.Execute(ITEM_TYPE_OPNAME); //frmTrsItemShipmentList
        400: Form.Execute; //TRANS_TYPE_PEMBAYARAN
...
//    1002:
//    begin
//      Application.CreateForm(TfrmMyPrintPreview, frmMyPrintPreview);
//      temp:= frmMyPrintPreview;
//    end;
    ...
    202:
    begin
      Application.CreateForm(TfrmMstRak, frmMstRak);
      temp:= frmMstRak;
    end;
    ...
    211:
    begin
      Application.CreateForm(TfrmMstPaketList, frmMstPaketList);
      temp:= frmMstPaketList;
    end;

    212:
    begin
      Application.CreateForm(TfrmMstBarcode, frmMstBarcode);
      temp:= frmMstBarcode;
    end;

    213:
    begin
      Application.CreateForm(TfrmMstProduk, frmMstProduk);
      temp:= frmMstProduk;
    end;
    ...
    330:
    begin
      Application.CreateForm(TfrmTrsSalesList, frmTrsSalesList);
      temp:= frmTrsSalesList;
    end;

    SALES_TYPE_BOS:
    begin
      Application.CreateForm(TfrmTrsSales2, frmTrsSales2);
      temp:= frmTrsSales2;
    end;
    ...
    TRANS_TYPE_PEMBAYARAN, TRANS_TYPE_PIUTANG:
    begin
      Application.CreateForm(TfrmTrsPembayaran, frmTrsPembayaran);
      temp:= frmTrsPembayaran;
    end;

    TRANS_TYPE_DANA_INPUT, TRANS_TYPE_DANA_OUTPUT:
    begin
      Application.CreateForm(TfrmTrsAkun, frmTrsAkun);
      temp:= frmTrsAkun;
    end;
    ...
    502, 504:
    begin
      Application.CreateForm(TfrmReportItemQty, frmReportItemQty);
      temp:= frmReportItemQty;
    end;
    ...
    520:
    begin
      Application.CreateForm(TfrmReportDebt, frmReportDebt);
      temp:= frmReportDebt;
    end;
