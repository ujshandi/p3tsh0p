...
//==============================================================================
  ...
  public
    ...
    class function LoadObatKF(): TMysqlResult;
    class function countKF(): integer;
    //class function countFromDB2(): integer;
...
begin
  case vTipe of
    ...
    RELASI_TYPE_SUPPLIER: prefix:= UpperCase(Copy(FRelationName, 1,1))+'.';
  ...
  Result:= ExecTransaction(
    'insert into mst_relation '+
      '(relation_type, relation_code, relation_name, address1, address2, post_code, '+
       'city, phone1, phone2, contact_person, npwp, due_date) '+
    'values ('+
...
begin
  buffer:= OpenSQL(
    ...
    'post_code, city, phone1, phone2, due_date, current_debt, contact_person, npwp '+
    'from mst_relation '+
...
begin
  ...
  'barcode, struk_name, current_stock, merk, avg_price '+
...
begin
  sqL:=
  'select i.item_id, i.item_code, i.item_name, barcode, current_stock, avg_price, dosis '+
  'from mst_item i '+
  IfThen(FItemId <> 0, 'where item_id = '+FormatSQLNumber(FItemId), 'where barcode = '+FormatSQLString(FBarcode));//+
//  IfThen(FVendorID <> 0, ' and vendor_id = '+FormatSQLNumber(FVendorID));
  ...
  if Result then begin
    with buffer do begin
      ...
      FDosis := BufferToFloat(FieldValue(6));
      FItemConversion.Clear;
      FItemConversion.Add;
      FItemConversion[0].ItemId:= FItemId;
...
begin
  sqL:=
  'select i.item_id, i.item_code, i.item_name, barcode '+
...
begin
  sqL:=
    'update mst_item SET'+
      ...
      ' kategori ='+IfThen(FKategori <> 0, FormatSQLNumber(FKategori), FormatSQLNULL)+','+
...
class function TMstItemArr.countKF: integer;
var sqL, filter: string;
begin
  if GlobalFilter.Name <> '' then
    filter:= filter + ' and NAMA_OBAT like '+FormatSQLString('%'+GlobalFilter.Name+'%');
  if GlobalFilter.FString1 <> '' then
    filter:= filter + ' and KODE_OBAT like '+FormatSQLString('%'+GlobalFilter.FString1+'%');
  if GlobalFilter.FString2 <> '' then
    filter:= filter + ' and barcode like '+FormatSQLString('%'+GlobalFilter.FString2+'%');
  if filter <> '' then filter:= ' where ' + copy(filter, 5, length(filter));

  sqL:= 'select count(*) from test.historyobat '+filter;
  result:= getIntegerFromSQL(sqL);
end;
...
begin
  ...
  sqL:=
  'select distinct i.item_id, i.barcode, i.item_name, i.current_stock, i.avg_price, '+
  'c.conv_id, c.conversion, u.mst_code, purchase_price '+
  'from mst_item i, mst_item_conversion c, mst_master u '+
...
begin
  ...
  if GlobalFilter.Numeric3 <> 0 then
    filter:= filter + ' and i.kategori ='+FormatSQLNumber(GlobalFilter.Numeric3);
 ...
 // 'k.purchase_price, k.item_discount, k.item_tax, k.net_price, i.avg_price, k.cost_price, k.sales_price, k.sales_price2, '+
  ...
  '(select mst_code from mst_master where mst_id = i.unit_id limit 1) as sat, '+
  'dosis, expire_date '+
  'from mst_item i,  point_maping p '+
  IfThen(GlobalFilter.Numeric2<>0, ', mst_rak_item r, mst_rak_maping rm ')+
...
begin
  ...
  sqL:=
  'select distinct i.item_id, i.barcode, i.item_name, i.current_stock, i.avg_price from mst_item i, point_maping p '+
  'where i.specification = p.code_id '+ fil;
...
class function TMstItemArr.LoadObatKF: TMysqlResult;
var sqL, filter: string;
begin
  if GlobalFilter.Name <> '' then
    filter:= filter + ' and NAMA_OBAT like '+FormatSQLString('%'+GlobalFilter.Name+'%');
  ...
  if GlobalFilter.FString2 <> '' then
    filter:= filter + ' and barcode like '+FormatSQLString('%'+GlobalFilter.FString2+'%');
  if filter <> '' then filter:= ' where ' + copy(filter, 5, length(filter));
  filter:= filter + ' order by NAMA_OBAT ';
  if GlobalFilter.FOffset <> 0 then
    filter:= filter +
    ' limit '+FormatSQLNumber(GlobalFilter.FLimit)+ ', '+FormatSQLNumber(GlobalFilter.FOffset);

  sqL:= 'select * from test.historyobat '+filter;
  result:= OpenSQL(sqL);
