...
type
  TfrmMstAnimalList = class(TDockForm)
    ToolBar1: TToolBar;
    tbtNew: TToolButton;
...
var
  frmMstAnimalList: TfrmMstAnimalList;
...
procedure TfrmMstAnimalList.Execute(id: integer);
begin
  inherited;
...
procedure TfrmMstAnimalList.InitFilter;
begin
  GlobalPeriode.Reset;
...
procedure TfrmMstAnimalList.InitForm;
begin
  InitGrid;
  InitFilter;
end;

procedure TfrmMstAnimalList.InitGrid;
begin
...
procedure TfrmMstAnimalList.LoadData;
...
procedure TfrmMstAnimalList.SetFilter;
begin
...
procedure TfrmMstAnimalList.gridGetAlignment(Sender: TObject; ARow,
...
procedure TfrmMstAnimalList.tbtNewClick(Sender: TObject);
begin
  if not TSystemAccess.isCan(CAN_ADD,AktiveControl.Tag) then exit;
  frmMstAnimal.Execute(0);
end;

procedure TfrmMstAnimalList.tbtDetailClick(Sender: TObject);
begin
...
procedure TfrmMstAnimalList.FormClose(Sender: TObject;
...
procedure TfrmMstAnimalList.ToolButton5Click(Sender: TObject);
begin
  InitFilter
end;

procedure TfrmMstAnimalList.tbtRefreshClick(Sender: TObject);
begin
  LoadData;
end;

procedure TfrmMstAnimalList.tbtPrintClick(Sender: TObject);
begin
...
procedure TfrmMstAnimalList.Aktivasi1Click(Sender: TObject);
begin
  if not TSystemAccess.isCan(CAN_DELETE,AktiveControl.Tag) then exit;
  TMstItem.activasi(grid.Ints[colId, grid.Row])
end;

procedure TfrmMstAnimalList.Button1Click(Sender: TObject);
var ID: integer;
begin
...
procedure TfrmMstAnimalList.gridRowChanging(Sender: TObject; OldRow,
...
procedure TfrmMstAnimalList.gridGetCellColor(Sender: TObject; ARow,
...
procedure TfrmMstAnimalList.gridDblClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
  tbtDetail.Click
end;

procedure TfrmMstAnimalList.ToolButton3Click(Sender: TObject);
begin
  if not TSystemAccess.isCan(CAN_DELETE,AktiveControl.Tag) then exit;
