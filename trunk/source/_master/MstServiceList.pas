...
type
  TfrmMstServiceList = class(TDockForm)
    ToolBar1: TToolBar;
    tbtNew: TToolButton;
...
var
  frmMstServiceList: TfrmMstServiceList;
...
procedure TfrmMstServiceList.Execute(id: integer);
begin
  inherited;
...
procedure TfrmMstServiceList.InitFilter;
begin
  GlobalPeriode.Reset;
...
procedure TfrmMstServiceList.InitForm;
begin
  InitGrid;
  InitFilter;
end;

procedure TfrmMstServiceList.InitGrid;
begin
...
procedure TfrmMstServiceList.LoadData;
...
begin
  try
    ...
    for i:= 1 to item.RecordCount do begin
      ...
      grid.Cells[colJenis,i]:= BufferToString(item.FieldValue(5));
      item.MoveNext;
    end;
...
procedure TfrmMstServiceList.SetFilter;
begin
  GlobalFilter.Reset;
...
procedure TfrmMstServiceList.gridGetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
  if (ACol in[colCode, colName,colnotes,colJenis]) then HAlign:= taLeftJustify
  else HAlign:= taRightJustify
end;

procedure TfrmMstServiceList.tbtNewClick(Sender: TObject);
begin
...
procedure TfrmMstServiceList.tbtDetailClick(Sender: TObject);
begin
...
procedure TfrmMstServiceList.FormClose(Sender: TObject;
...
procedure TfrmMstServiceList.ToolButton5Click(Sender: TObject);
begin
  InitFilter
end;

procedure TfrmMstServiceList.tbtRefreshClick(Sender: TObject);
begin
  LoadData;
end;

procedure TfrmMstServiceList.tbtPrintClick(Sender: TObject);
begin
...
procedure TfrmMstServiceList.Aktivasi1Click(Sender: TObject);
begin
  TMstService.activasi(grid.Ints[colId, grid.Row])
end;

procedure TfrmMstServiceList.Button1Click(Sender: TObject);
var ID: integer;
begin
...
procedure TfrmMstServiceList.gridRowChanging(Sender: TObject; OldRow,
...
procedure TfrmMstServiceList.gridGetCellColor(Sender: TObject; ARow,
...
procedure TfrmMstServiceList.gridDblClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
  tbtDetail.Click
end;

procedure TfrmMstServiceList.ToolButton3Click(Sender: TObject);
begin
