...
begin
  ...
  result.fRecordCount:=result.fdata.rows;
  if ae then
  ...
  faffected_rows:= result.fRecordCount;
  finally
    ffields:=nil;
    ffield_count:=0;
    fstatus:=MYSQL_STATUS_READY;
...
begin
  ...
  result.MoveNext;//read first row
...
begin
  ...
  if (fhandle<>nil) and (fhandle.fstatus = MYSQL_STATUS_USE_RESULT) then
    begin //is there anything on the buffer?
      ...
      fhandle.faffected_rows:= fRecordCount;
    end;
...
begin
  if (ftype=rtStored)then
    ...
    begin
      ...
      while (fLastRow=0) do
        begin
          inc(fRecordCount);//we got a new record we can exit
          fLastRow:=fhandle.read_one_row(ffieldscount,frow,flengths);
        end;
      fRecNo:=fRecordCount-1;
      if assigned(fhandle) then
...
procedure TMysqlResult.MoveNext;
begin
  if ftype=rtUsed then
    begin
      FBof:=fRecordCount=0;
